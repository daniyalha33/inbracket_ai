...
from external_libs.pointops import pointops
from .utils import *
...
class PointTransformerLayer(nn.Module):
    ...
    def forward(self, pxo) -> torch.Tensor:
        ...
        batch_size = len(o)
        x_k = pointops.queryandgroup(p, p, x_k, batch_size, self.nsample, use_xyz=True)  # (n, nsample, 3+c)
        x_v = pointops.queryandgroup(p, p, x_v, batch_size, self.nsample, use_xyz=False)  # (n, nsample, c)
...
class TransitionDown(nn.Module):
    ...
    def forward(self, pxo):
        ...
        if self.stride != 1:
            ...
            idx = pointops.furthestsampling_gpu(p, batch_size, n_o[0]) # (m)
            n_p = p[idx.long(), :]  # (m, 3)
            x = pointops.queryandgroup(p, n_p, x, batch_size, self.nsample, use_xyz=True) # (m, nsample, 3+c)
...
class TransitionUp(nn.Module):
    ...
    def forward(self, pxo1, pxo2=None):
        if pxo2 is None:
            ...
            x = self.linear1(x1) + pointops.interpolation(p2, p1, self.linear2(x2), batch_size)
        return x
